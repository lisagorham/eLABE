...
#This will be submitted to Neuron.
...
setwd("put whatever your directory is here")


#Load in data files (change to deidentified last) ----------------------------------------------

#demographic information
demographics <- read_xlsx("path/demographics.xlsx")

#whole brain surface area/volume/GI measures
complete_data <- read.csv("path/dataforfigures4and5.csv")

#whole brain expansion numbers
figure1data <- read.csv("path/dataforfigure1.csv")

#parcellated expansion numbers (Glasser)
birthcohort <- read.csv("path/dataforfigures2and3.csv")

#raw data for deriving thrive
rawdata <- read_xlsx("path/thrive_raw_data.xlsx")

#parcellated surface area data (Glasser)
year2SA <- read.csv("path/inf_2yo_areas_Glasser.csv")
year3SA <- read.csv("path/inf_3yo_areas_Glasser.csv")

#components of PSD variable for method section
data <- read.csv("path/PSD_subcomponents.csv")
...
#Figure 4/Supplementary Table 10- Expansion Thrive*PSD ----------------
...
#Interpreting PSD of 0 for the method section ----------------------------

data$INR_T1 <- 10^(data$LOG_IN1)
data$INR_T2 <- 10^(data$LOG_IN2)
data$INR_T3 <- 10^(data$LOG_IN3)
data$INR_avg <- rowMeans(
  data[, c("INR_T1", "INR_T2", "INR_T3")],
  na.rm = TRUE
)
data$INR_avg[is.nan(data$INR_avg)] <- NA

fit_ADI <- lm(ADI ~ disadv_prenatal, data = data)
fit_INR <- lm(INR_avg ~ disadv_prenatal, data = data)

newdat <- data.frame(disadv_prenatal = 0)
